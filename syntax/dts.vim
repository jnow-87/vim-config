...
syn region dts_arch_block start="\s*=\s*{" end="}" contains=dts_comment,@dts_preproc,dts_arch_key,dts_device fold

syn keyword dts_section memory nextgroup=dts_memory_block
syn region dts_memory_block start="\s*=\s*{" end="}" contains=dts_comment,@dts_preproc,dts_device fold

syn keyword dts_section devices nextgroup=dts_devices_block
syn region dts_devices_block start="\s*=\s*{" end="}" contains=dts_comment,@dts_preproc,dts_device fold
...
" preprocessor
syntax match	dts_c_pp			"^\s*\(%:\|#\)\s*include\ze\s*[<\"].*[>\"]"
